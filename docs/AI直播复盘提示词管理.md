...
- 对高频弹幕 Top50 做价值筛选
- 删除灌水、刷屏、无意义重复
- 当前还会在系统侧先剔除重复发送超过 10 次的自动福袋类弹幕
- 主分析流程读取的 `highFrequencyComments` 也与该阈值保持一致，不再额外注入未过滤的高频刷屏话术
...
- 复盘里的时间表达优先使用“开播后HH:MM:SS”或“开播后HH:MM:SS-开播后HH:MM:SS”的相对时间范围，不直接展示北京时间，方便团队跨场次复盘时按流程节奏联想
- 看到 `04:29:00` 这类值时，必须理解为“开播后 4 小时 29 分 00 秒”，不能误写成“凌晨/下半夜”
