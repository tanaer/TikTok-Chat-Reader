...
const PROMPT_TEMPLATES = {
    ...
    session_recap_review: {
        ...
        variables: ['sessionDataJson', 'sessionRecapScoreBenchmarkJson', 'sessionRecapNewAttentionCustomersJson'],
        defaultContent: [
            ...
            '5. 礼物表现判断必须结合系统给出的单场及格线基准，不能只看绝对礼物值。',
            '6. 如果系统给出了“本场新增关注信号（Heart Me）”，要把它识别为关系推进机会点，优先写入 highlights / actions。',
            '7. 涉及时间表达时，优先沿用输入里的“开播后HH:MM:SS”或“开播后HH:MM:SS-开播后HH:MM:SS”相对时间范围，不要改写成北京时间、具体日期时间、凌晨/后半夜/黄金时段这类自然时段判断。',
            '8. 看到“04:29:00”这类时间时，默认将其理解为“开播后 4 小时 29 分 00 秒”，不是凌晨 4 点 29 分。',
            '',
            ...
            '- nickname、uniqueId、totalGiftValue、giftCount、enterTime、leaveTime 等字段优先沿用输入，不要编造；如果输入给的是“开播后HH:MM:SS”或相对时间范围，就继续沿用相对时间表达。',
            '- 严禁把“开播后04:29:00”理解成凌晨 4 点 29 分，严禁据此写出“下半夜 / 深夜 / 凌晨时段”这类自然时段结论；除非输入明确提供真实北京时间与时段信息。',
            '- 每个客户对象都必须与输入里的同一条客户记录保持一致，uniqueId 如有提供必须原样保留；不要把 A 用户的话术、原因、动作写到 B 用户上。',
            '- 客户卡片里的 keyBehavior / maintenanceSuggestion / riskReason / recoveryStrategy 重点写“判断 + 原因 + 动作”，不要机械复述输入里的模板句。',
            '- 页面会单独展示系统统计事实，所以客户卡片里的文案不要重复堆砌具体钻石数、弹幕数、点赞数、进房数；除非该数字本身就是判断关键，否则优先输出解释和策略。',
            '- 如果输入显示该用户本场已送礼，就不要在文案里写“未出手/没出手/零礼物”；如果输入显示该用户本场未送礼，也不要写成“已送礼/本场送了”。',
    ...
    customer_analysis_review: {
        ...
        sortOrder: 30,
        title: 'AI客户分析 · 主分析流程',
        description: '用于房间详情 / 历史排行榜中的客户价值深度挖掘，基于结构化客户上下文与最近弹幕语料输出直接结论、风险判断、维护策略与话术。',
        ...
        defaultContent: [
            ...
            '3. 重点结论、下一步动作、主播承接话术、注意事项必须优先给运营可直接执行的话。',
            '4. modelEvidence / contributionEvidence / riskEvidence / interactionEvidence 仅作补充依据字段，能短就短，不要写成长篇说明。',
            '',
            '输出要求：',
            ...
            '4.1 贡献占比优先写成百分比，并明确主体，例如“近30天该客户总贡献里投向本房的占比约34.27%”。',
            ...
            '6. summary 控制在 120 字以内，格式尽量接近“本房价值判断 + 当前主要风险/机会 + 下一步动作”，直接说结论，不要堆模型解释。',
            ...
            '9. modelEvidence 重点写 LRFM / ABC / 模型分层判断，返回 0-2 条；如果 summary 和 keySignals 已说清，可留空数组。',
            '10. contributionEvidence 重点写贡献占比、礼物排名、价值强弱，返回 0-2 条；如果信息容易产生歧义，就改写清楚再输出。',
            '11. riskEvidence 重点写趋势、跨房增长、沉默/流失/活跃变化，返回 0-2 条；没有必要不要硬凑。',
            '12. interactionEvidence 重点写弹幕语气、互动深度、行为信号；如果 chatCorpusText 为空，就返回空数组，不要虚构。',
            '13. evidence 是兼容字段，可返回 0-1 条补充事实；不要把前 4 组证据重复堆进去。',
