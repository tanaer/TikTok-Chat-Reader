...
function showSessionRecapTeaser(recap = {}) {
    ...
    const rightPanel = isLive
        ? `
            ...
            <div class="mt-4 rounded-box bg-base-200/70 px-4 py-3 text-sm opacity-80">归档后可解锁：直播摘要 / 关键时刻 / 价值客户 / 建议动作。</div>
        `
        : `
    ...
    setSessionRecapEmptyHtml(buildSessionRecapInfoCard({
        ...
        chips: isLive
            ? ['归档后生成更稳', '直播摘要', '价值客户', '关键时刻']
            : [startTime ? `场次时间 ${startTime}` : '单场归档复盘', '直播摘要', '关键时刻', '价值客户'],
        rightPanel,
...
function showSessionRecapGenerating(delayMs = 7000) {
    ...
    setSessionRecapEmptyHtml(buildSessionRecapInfoCard({
        ...
        chips: ['直播摘要生成中', '关键时刻提炼中', '价值客户识别中', '通常 5-10 秒'],
        rightPanel,
        tone: 'primary'
    }));
...
function applySessionRecapData(recap) {
    ...
    if (meta) {
        ...
        else meta.textContent = `生成后将按单场消耗 ${Number(recap.pointCost || 10)} 点；下次进入直接显示。`;
    }
