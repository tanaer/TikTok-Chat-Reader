...
URL地址：https://cdn2.futooncdn.com/submit.php
...
| ------------ | -------------- | ---- | ------ | ---------------------------------- | ------------------------------------------------------------ |
| 商户ID       | pid            | 是   | Int    | 1001                               |                                                              |
| 支付方式     | type           | 否   | String | alipay                             | [支付方式列表](https://cdn2.futooncdn.com/doc.html#pay4)     |
| 商户订单号   | out\_trade\_no | 是   | String | 20160806151343349                  |                                                              |
| 异步通知地址 | notify\_url    | 是   | String | http://www.pay.com/notify\_url.php | 服务器异步通知地址                                           |
| 跳转通知地址 | return\_url    | 是   | String | http://www.pay.com/return\_url.php | 页面跳转通知地址                                             |
| 商品名称     | name           | 是   | String | VIP会员                            | 如超过127个字节会自动截取                                    |
| 商品金额     | money          | 是   | String | 1.00                               | 单位：元，最大2位小数                                        |
| 业务扩展参数 | param          | 否   | String | 没有请留空                         | 支付后原样返回                                               |
| 签名字符串   | sign           | 是   | String | 202cb962ac59075b964b07152d234b70   | 签名算法[点此查看](https://cdn2.futooncdn.com/doc.html#pay3) |
...
URL地址：https://cdn2.futooncdn.com/mapi.php
...
| -------------------------------------------------------- | -------------- | ---- | ------ | ---------------------------------- | ------------------------------------------------------------ |
| 商户ID                                                   | pid            | 是   | Int    | 1001                               |                                                              |
| 支付方式                                                 | type           | 是   | String | alipay                             | [支付方式列表](https://cdn2.futooncdn.com/doc.html#pay4)     |
| 商户订单号                                               | out\_trade\_no | 是   | String | 20160806151343349                  |                                                              |
| 异步通知地址                                             | notify\_url    | 是   | String | http://www.pay.com/notify\_url.php | 服务器异步通知地址                                           |
| 跳转通知地址                                             | return\_url    | 否   | String | http://www.pay.com/return\_url.php | 页面跳转通知地址                                             |
| 商品名称                                                 | name           | 是   | String | VIP会员                            | 如超过127个字节会自动截取                                    |
| 商品金额                                                 | money          | 是   | String | 1.00                               | 单位：元，最大2位小数                                        |
...
| 传入用户所使用的浏览器                                   |                |      |        |                                    |                                                              |
| 或设备类型，默认为pc                                     |                |      |        |                                    |                                                              |
| [设备类型列表](https://cdn2.futooncdn.com/doc.html#pay5) |                |      |        |                                    |                                                              |
| 业务扩展参数                                             | param          | 否   | String | 没有请留空                         | 支付后原样返回                                               |
| 签名字符串                                               | sign           | 是   | String | 202cb962ac59075b964b07152d234b70   | 签名算法[点此查看](https://cdn2.futooncdn.com/doc.html#pay3) |
| 签名类型                                                 | sign\_type     | 是   | String | MD5                                | 默认为MD5                                                    |
...
| 订单号        | trade\_no | String | 20160806151343349                               | 支付订单号                                              |
| 支付跳转url   | payurl    | String | https://cdn2.futooncdn.com/pay/wxpay/202010903/ | 如果返回该字段，则直接跳转到该url支付                   |
...
| ------------ | -------------- | ---- | ------ | -------------------------------- | ------------------------------------------------------------ |
| 商户ID       | pid            | 是   | Int    | 1001                             |                                                              |
| 易支付订单号 | trade\_no      | 是   | String | 20160806151343349021             | Futoon Pay订单号                                             |
...
1、将发送或接收到的所有参数按照参数名ASCII码从小到大排序（a-z），sign、sign\_type、和空值不参与签名！
...
URL地址：https://cdn2.futooncdn.com/api.php?act=query&pid={商户ID}&key={商户密钥}
...
URL地址：https://cdn2.futooncdn.com/api.php?act=settle&pid={商户ID}&key={商户密钥}
...
URL地址：https://cdn2.futooncdn.com/api.php?act=order&pid={商户ID}&key={商户密钥}&out\_trade\_no={商户订单号}
...
| 易支付订单号 | trade\_no      | String | 2016080622555342651 | Futoon Pay订单号                                         |
| 商户订单号   | out\_trade\_no | String | 20160806151343349   | 商户系统内部的订单号                                     |
| 第三方订单号 | api\_trade\_no | String | 20160806151343349   | 支付宝微信等接口方订单号                                 |
| 支付方式     | type           | String | alipay              | [支付方式列表](https://cdn2.futooncdn.com/doc.html#pay4) |
...
URL地址：https://cdn2.futooncdn.com/api.php?act=orders&pid={商户ID}&key={商户密钥}
...
URL地址：https://cdn2.futooncdn.com/api.php?act=refund
...
| 商户ID       | pid            | 是       | Int    | 1001                             |                                                            |
| 商户密钥     | key            | 是       | String | 89unJUB8HZ54Hj7x4nUj56HN4nUzUJ8i |                                                            |
| 易支付订单号 | trade\_no      | 特殊可选 | String | 20160806151343349021             | 易支付订单号                                               |
| 商户订单号   | out\_trade\_no | 特殊可选 | String | 20160806151343349                | 订单支付时传入的商户订单号，商家自定义且保证商家系统中唯一 |
| 退款金额     | money          | 是       | String | 1.50                             | 少数通道需要与原订单金额一致                               |

注：trade\_no、out\_trade\_no 不能同时为空，如果都传了以trade\_no为准
...
URL地址：https://cdn2.futooncdn.com/api.php?act=paytype&pid={商户ID}&key={商户密钥}
...
| ---------- | ------ | ------ | -------- | --------------------- |
| 返回状态码 | code   | Int    | 1        | 1为成功，其它值为失败 |
| 返回信息   | msg    | String | 查询成功 |                       |
| 返回数据   | data   | Array  |          | 支付方式列表          |

data.item

| 字段名       | 变量名   | 类型   | 示例值 | 描述                                            |
| ------------ | -------- | ------ | ------ | ----------------------------------------------- |
| 支付类型     | name     | String | wxpay  | wxpay/alipay/qqpay/bank/jdpay/paypal            |
| 支付类型描述 | showname | String | 微信   | 微信支付/支付宝/QQ钱包/网银支付/京东支付/PayPal |